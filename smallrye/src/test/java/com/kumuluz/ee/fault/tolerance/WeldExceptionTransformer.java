...
/**
 * WELD throws exceptions that the TCK is looking for as a suppressed exception in {@link DefinitionException}. This
 * transformer unwraps it.
 *
 * @author Urban Malc
 * @since 2.0.0
 */
public class WeldExceptionTransformer implements DeploymentExceptionTransformer {
    ...
    public Throwable transform(Throwable exception) {
        if (exception instanceof DefinitionException) {
            return exception.getSuppressed()[0];
        }
